...
export class ChatController {
  ...
  // async createConversation(@Request() req: { user }, @Body(new ValidationPipe()) emailsDto: EmailsDto) {
  //   return await this.chatService.createConversation(req.user, emailsDto.emails);
  // }

  @UseGuards(AuthGuard('jwt'))
  @Post('list-message')
  async getListMessage(@Request() req, @Body() body: ConversationDto) {
    return await this.chatService.getListMessage(req.user.id, body.conversation_id);
  }
}
