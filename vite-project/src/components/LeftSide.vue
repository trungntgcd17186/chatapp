...
import PopupInvite from './PopupInvite.vue';
...
<template>
  <PopupInvite :open="open" :onClose="() => open = false" />
    ...
    <div class="flex flex-col justify-between h-full">
      <div class="chat-history overflow-y-auto max-h-[calc(100vh-154px)] mt-[100px] w-[359px] px-2">
        <ItemChat
          ...
          :nameUserLastMessage="noti?.[item.id]?.userFirstName || item?.messages?.[0]?.user?.first_name"
      ...
      <div class="p-2 h-[54px] border-t border-[#0000001a]">
        <IconWrapper :isHideBg="true" :width="'full'" :height="'full'" :rounded="'rounded-lg'">
